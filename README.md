...
The app is a static Vite SPA (no server-side code), hosted on Vercel.

Import the repo in the [Vercel dashboard](https://vercel.com/new) — Vercel
auto-detects Vite (build command `yarn build`, output directory `dist`). Its Git
integration deploys every push to `master` to production and gives every other
branch/PR its own preview URL, so there is no deploy step in CI.
...
> **Node 20+ required** (see `engines` in `package.json`) — the PWA
> service-worker build hashes in a worker thread that needs the Web Crypto
> global, which Node exposes to workers only from v20 on.
